  ...
  moveTouch = (nativeEvent, gestureState) => {
    ...
    if (!this.isUpdating) {
      ...
      for (let layout of layouts) {
        ...
        let cb = this.move(curLayout.key, layout.key, vy, curLayout.y !== layout.y)
        if (cb) {
      ...
      if (!this.isUpdating && nextLineLastLayout &&
        ...
        let cb = this.move(curLayout.key, nextLineLastLayout.key, vy, curLayout.y !== nextLineLastLayout.y)
        if (cb) {
          finalCallbacks.push(cb);
        }
      }
    ...
    let finalCallback = () => {
      if (finalCallbacks.length > 1) {
        console.warn(`[WARNING] ${finalCallbacks.length} move operations at one time?`);
      }
      finalCallbacks.forEach(cb => cb());
    }
